...
Para más detalles consulta '¿Qué es el aviso "conflicto de dependencias"?' en [Dudas de aplicaciones]({{ site.url }}/anexos/dudas-aplicaciones/).
...
La terminal (o "consola") viene incluida para cualquier situación. No consume recursos, basta escribir el shell y el comando. Por ejemplo, para [actualizar los repositorios]({{ site.url }}{{ site.baseurl }}/tips/mirror/) escribo:

{% include alert terminal='apt update' %}

La terminal no te hace genial como [Jonny Lee Miller y Angelina Jolie en Hackers](https://en.wikipedia.org/wiki/Hackers_(film)). Si has usado "Símbolo de sistema" en Windows, las órdenes son los más; tendrás que escribir `help` para conocer el listado de órdenes de un shell.
